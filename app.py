...
# ============= AI GENERATION (with per‑slot task hints) =============
def generate_schedule(user_tasks, slot_tasks, employee_name, position, report_date, provider, api_key, model_name, lunch_hour, progress_callback=None):
    ...
    # Prepare per-slot instructions from the user's inputs
    # slot_tasks is a dict mapping slot -> user-provided task string
    # We'll create a list of slot: task lines for the prompt
    slot_task_lines = []
    for slot in slot_labels:
        if slot == lunch_label:
            continue  # lunch is fixed, no need to include
        task = slot_tasks.get(slot, "").strip()
        if task:
            # Include the user's hint
            slot_task_lines.append(f"- {slot}: {task}")
        else:
            slot_task_lines.append(f"- {slot}: (no specific task)")

    slot_hints = "\n".join(slot_task_lines)

    # Also include any free-text tasks if present
    free_text = user_tasks.strip() if user_tasks else ""
...
The user has provided hints for each slot (or "no specific task"):
{slot_hints}

Additionally, they may have provided a general task summary:
{free_text}

Your job is to generate a complete schedule for all slots. For each slot (except lunch), you must:
- If the user provided a hint (e.g., "Posted stories"), use that as the activity or as a basis for the activity.
- Write a concise, professional description (1‑2 sentences) that explains the work done.
- If the user wrote "-" or left it blank, you can either set activity and description to "-" (if they explicitly wrote "-") or intelligently fill it based on other tasks.
...
with left_col:
    ...
    # ---- Per‑slot task inputs ----
    st.markdown("### 📋 Task per Time Slot")
    st.caption("Type your task for each slot. Leave blank if you want the AI to fill it intelligently. Use '-' to indicate nothing was done.")

    # Get time slots based on current lunch hour
    time_slots = get_time_slots(lunch_hour)
    lunch_label = time_slots[lunch_hour - 10]

    # We'll store user inputs in session state
    if "slot_tasks" not in st.session_state:
        st.session_state.slot_tasks = {slot: "" for slot in time_slots if slot != lunch_label}

    # Rebuild if lunch changed
    current_slots = list(st.session_state.slot_tasks.keys())
    if set(current_slots) != set([s for s in time_slots if s != lunch_label]):
        # Update the dict, preserving values for slots that still exist
        new_tasks = {}
        for slot in time_slots:
            if slot == lunch_label:
                continue
            if slot in st.session_state.slot_tasks:
                new_tasks[slot] = st.session_state.slot_tasks[slot]
            else:
                new_tasks[slot] = ""
        st.session_state.slot_tasks = new_tasks

    # Display inputs for each slot
    for slot in time_slots:
        if slot == lunch_label:
            # Display lunch slot as non-editable
            st.text_input(f"🕒 {slot}", value="Lunch Break", disabled=True)
        else:
            key = f"slot_task_{slot}"
            # Use text_input for each slot
            # We'll use session state to store and update
            # We'll create a separate widget for each slot
            current_val = st.session_state.slot_tasks.get(slot, "")
            new_val = st.text_input(f"🕒 {slot}", value=current_val, key=key)
            st.session_state.slot_tasks[slot] = new_val

    # Optional free-text area for additional tasks
    st.markdown("### 📝 Additional Tasks (Optional)")
    additional_tasks = st.text_area("Any extra tasks not assigned to a specific slot?",
                                     height=80,
                                     placeholder="e.g., answered emails, reviewed reports...",
                                     key="additional_tasks")

    # Quick templates (fill the slot inputs)
    templates_quick = {
        "Feng Shui & Content": {
            "10:00 am to 11:00 am": "Posted Feng Shui stories",
            "11:00 am to 12:00 pm": "Created post on July animal signs",
            "12:00 pm to 1:00 pm": "Started editing Kedarnath reel",
            "2:00 pm to 3:00 pm": "Created 15 AI creatives for reel",
            "3:00 pm to 4:00 pm": "Continued editing reel",
            "4:00 pm to 5:00 pm": "Reviewed performance",
            "5:00 pm to 6:00 pm": "Planned next steps"
        },
        "Meetings & Documentation": {
            "10:00 am to 11:00 am": "Team sync meeting",
            "11:00 am to 12:00 pm": "Wrote meeting notes",
            "12:00 pm to 1:00 pm": "Follow-up emails",
            "2:00 pm to 3:00 pm": "Project planning",
            "3:00 pm to 4:00 pm": "Client call",
            "4:00 pm to 5:00 pm": "Prepared status report",
            "5:00 pm to 6:00 pm": "Reviewed and finalized"
        },
        "Development & Testing": {
            "10:00 am to 11:00 am": "Fixed bugs",
            "11:00 am to 12:00 pm": "Developed new feature",
            "12:00 pm to 1:00 pm": "Code review",
            "2:00 pm to 3:00 pm": "Wrote tests",
            "3:00 pm to 4:00 pm": "Deployed to staging",
            "4:00 pm to 5:00 pm": "Updated documentation",
            "5:00 pm to 6:00 pm": "Sprint planning"
        },
        "Custom": {}
    }
    ...
    if selected_template_quick != "Custom" and templates_quick[selected_template_quick]:
        if st.button("📋 Load template", use_container_width=True):
            # Fill the slot_tasks dict with template data
            for slot, task in templates_quick[selected_template_quick].items():
                # Find the slot in the current time slots (case-insensitive)
                for s in st.session_state.slot_tasks.keys():
                    if s.lower() == slot.lower():
                        st.session_state.slot_tasks[s] = task
                        break
            st.rerun()
...
if generate_clicked or regenerate_clicked:
    if regenerate_clicked:
        ...
        # Additional tasks
        additional = st.session_state.get("additional_tasks", "").strip()
        # Build a combined input for the AI: we'll pass slot tasks and additional separately
        # We'll include them in the prompt as per the generate_schedule function
        # We need to pass slot_tasks as a dict.
        ...
        st.session_state.last_input = additional  # we'll use this as free text
        st.session_state.last_config = {
            "provider": provider,
            "api_key": api_key,
            "model": model_name,
    ...
    try:
        ...
        data = generate_schedule(st.session_state.last_input, slot_tasks, emp_used, pos_used, date_used,
                                 provider_used, api_key_used, model_used, lunch_hour_used, progress_callback=update_progress)
